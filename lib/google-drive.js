...
function getDriveClient() {
    ...
    if (!raw) {
        console.error('[MedGzuri] getDriveClient: No service account env var found');
        return null;
    }
    console.warn('[MedGzuri] getDriveClient: Service account JSON length:', raw.length);

    let credentials;
    try {
        credentials = JSON.parse(raw);
    } catch (parseErr) {
        console.error('[MedGzuri] Failed to parse Google service account JSON:', parseErr.message);
        console.error('[MedGzuri] First 50 chars:', raw.substring(0, 50));
        return null;
    }
    console.warn('[MedGzuri] getDriveClient: Parsed OK, project_id:', credentials.project_id, 'client_email:', credentials.client_email);

    // Fix escaped newlines in private_key (common Vercel env var issue)
